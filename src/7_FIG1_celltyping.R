...
Identify_Marker_TFs<-function(x,prefix,group_by,assay,pval_filt=1,assay_name){
      if (assay != "chromvar") {
    ...
    saveRDS(markers,file=paste0(prefix,".markers.",assay_name,".df.rds"))
    return(markers) 
}
...
plot_top_tf_markers<-function(x=out_subset,group_by,prefix,n_markers=20,order_by_idents=TRUE){
    ...
    markers<-list(
        ...
        Identify_Marker_TFs(x=x,group_by=group_by,assay="GeneActivity",assay_name="GeneActivity",prefix=prefix),
        Identify_Marker_TFs(x=x,group_by=group_by,assay="chromvar",assay_name="chromvar",prefix=prefix))
