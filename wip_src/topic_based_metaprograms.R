...
compare_topics<-function(x_name,y_name){
  ...
  lapply(colnames(titan_topic_genes[[x_name]]),function(xi){
    lapply(colnames(titan_topic_genes[[y_name]]),function(yj){
      ...
      length(intersect(unlist(titan_topic_genes[[x_name]][,xi]),unlist(titan_topic_genes[[y_name]][,yj]))),
      cosine_similarity(titan_topic_genes[[x_name]][,xi],titan_topic_genes[[y_name]][,yj]),
      jaccard_similarity(titan_topic_genes[[x_name]][,xi],titan_topic_genes[[y_name]][,yj]))
      })})
}
...
process_titan_topics<-function(dat,titan_objs,met,prefix,titan_path,sim_method="cosine",col_in=c("black","pink","white")){
  ...
  row.names(sim_mat)<-unique(paste(sim_mat$dat1,sim_mat$topic_dat1,sep="_"))#check that row is correct for this
