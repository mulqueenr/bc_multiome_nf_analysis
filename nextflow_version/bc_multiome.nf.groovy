...
process MERGE_SAMPLES_CALLPEAKS {
	...
	script:
		...
		for i in "\${samples_arr[@]}"; do cat \${i}"/outs/atac_fragments.tsv.gz"; done > merged_fragments.tsv.gz

		#sort and remove header for fragment files
		zcat merged_fragments.tsv.gz | \\
		grep -v "^#" | \\
		sort --parallel=${task.cpus} -T . -k1,1 -k2,2n -k3,3n - > merged_fragments_sorted.tsv

		gzip merged_fragments_sorted.tsv

		#run macs3 to call atac peaks
		
		macs3 \\
		callpeak -f BED \\
		-t merged_fragments_sorted.tsv.gz \\
		-g hs \\
		-n merged \\
		--max-count 1 \\
		-B \\
...
process MERGE_SAMPLE_AND_FILTER {
...
workflow {
	...
	/* Alternative where bed file is given rather than generated by merged sample, not running		
